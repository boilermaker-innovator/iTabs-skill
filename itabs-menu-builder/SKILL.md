...
### Val type: MUST be HTTP val
The val MUST be an HTTP val that returns a Response. Never put HTML directly in a .ts file.

```typescript
export default async function(req: Request): Promise<Response> {
  const html = '<!DOCTYPE html><html>...YOUR FULL HTML...</html>';
...
Do NOT use loops or innerHTML to render menu items. **Hardcode every item as static HTML.** Dynamic generation with string concatenation in Val.town breaks silently — quotes get mangled, onclick handlers fail, items don't render.

```javascript
// WRONG — dynamic, breaks in Val.town
coffeeItems.forEach(function(item) {
  el.innerHTML += '<div class="card" onclick="add(' + item.name + ')">' + item.name + '</div>';
});

// CORRECT — static, always works
// Just write out each card as plain HTML in the Response string
```
...
### Val.town Deploy
- Val type = **HTTP val** — returns `new Response(html, { headers: { 'Content-Type': 'text/html' } })`
- Never put raw HTML in a .ts file — it will throw a parse error
- Paste HTML directly into index.html (HTML val type) for standalone deploys
- Same URL every time after first deploy
- Test on phone immediately — mobile behaviour differs from desktop preview

### Static HTML — No Dynamic Generation
- **Never use loops or innerHTML to render menu items in Val.town**
- Hardcode every item card as static HTML in the Response string
- Dynamic generation breaks silently — quotes get mangled, onclick handlers fail
- More code but zero debugging — just works
